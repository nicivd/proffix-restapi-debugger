...
export class PxHttpService {
   ...
   * @param header String mit der Bezeichnung des Headers, der ausgelesen werden soll. Default: pxmetadata
   */
  public getWithMetaData<T>(endpoint: string, params?: object, header: string = "pxmetadata"): Observable<{ metadata?: any, data: T }> {
    return this.http.get<HttpResponse<T>>(this.getAbsolutUrl(endpoint), this.createRequestOption(params, 'response'))
      .pipe(map(response => {
        const metaData = response.headers ? JSON.parse(response.headers.get(header) ?? "null") : null;
        return { metadata: metaData, data: response.body as T };
      }));
  }
  ...
  /**
   * PATCH-Request auf die REST API
   * @param endpoint Endpunkt der aufgerufen wird z.B ADR/Adresse/1
   * @param body Daten-Objekt welches zum Server transportiert wird
   * @param params Object mit den Parametern welche dem Request mitgegeben werden sollen
   */
  public patch(endpoint: string, body: any, params?: object): Observable<void> {
  ...
  private createRequestOption(params?: object, observe: 'body' | 'response' = 'body'): { headers?: HttpHeaders, params?: HttpParams, observe?: any } {
    ...
    if (params) {
      for (const param in params) {
        if (params.hasOwnProperty(param) && (params as any)[param]) {
          if (requestOptions.params == null) {
            requestOptions.params = new HttpParams();
          }
          requestOptions.params = requestOptions.params.set(param, (params as any)[param]);
        }
      }
    }
